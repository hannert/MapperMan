...
import { addTagThunk } from '../../app/store-actions/editMapList';
...
export default function CollaboratorModal(props){
    ...
    function handleConfirm() {
        ...
        if (localTags?.length === currentTags?.length || localTags?.length < currentTags?.length){
            enqueueSnackbar("No new tags added!", {variant:'success', autoHideDuration:1000})
            toggleTagDialog();
        }
        else{
            for (var i = 0; i < localTags?.length; i++){
                console.log("adding tag " + localTags[i])
